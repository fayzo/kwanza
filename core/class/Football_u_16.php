...
class Football_u_16 extends Home{
    
    public function currentDatefootball_u_16Match($day,$currentdate,$futuredate)
    {
        ...
        $query= $mysqli->query("SELECT * FROM football_u_16 F
             ...
             <nav>
                ...
                <?php for ($i= 5; $i > 0; $i--) { 
                       ...
                       $date1 = strtotime("-$i day"); 
                        if (-$i == $day) { ?>
                    <li class="page-item active"><a class="page-link px-1  text-light" href="javascript:void(0)" onclick="Football_Home_FecthRequest('U-16',<?php echo -$i; ?>,'<?php echo date('Y-m-d', $date); ?>','<?php echo date('Y-m-d', $date1); ?>')" ><?php echo date('M d', $date); ?></a></li>
                <?php }else{ ?>
                       <li class="page-item"><a class="page-link px-1 main-active" href="javascript:void(0)" onclick="Football_Home_FecthRequest('U-16',<?php echo -$i; ?>,'<?php echo date('Y-m-d', $date); ?>','<?php echo date('Y-m-d', $date1); ?>')"><?php echo date('M d', $date);  ?></a></li>
                <?php } } ?>
                <?php for ($i= 0; $i < 5; $i++) { 
                       $date = strtotime("+$i day");
                       $date1 = strtotime("+$i day"); 
                         if ($i == $day) { ?>
                    <li class="page-item active"><a class="page-link px-1  text-light" href="javascript:void(0)" onclick="Football_Home_FecthRequest('U-16',<?php echo $i; ?>,'<?php echo date('Y-m-d', $date); ?>','<?php echo date('Y-m-d', $date1); ?>')" ><?php if ($i === 0) { echo 'Today';}else{ echo date('M d', $date); } ?></a></li>
                <?php }else{ ?>
                    <li class="page-item"><a class="page-link px-1 main-active" href="javascript:void(0)" onclick="Football_Home_FecthRequest('U-16',<?php echo $i; ?>,'<?php echo date('Y-m-d', $date); ?>','<?php echo date('Y-m-d', $date1); ?>')" ><?php echo date('M d', $date);  ?></a></li>
                <?php } } ?>
                </ul>
...
$football_u_16 = new Football_u_16();
/*
===========================================
         Notice
===========================================
