...
class Volleyball extends Home{
    
    public function VolleyballRecent_news($categories)
    {
    ...
    public function currentDateVolleyballMatch($day,$currentdate,$futuredate)
    {
        // if($day === 0){
        //   $dayz= 1;
        // }else{
        ...
        $query= $mysqli->query("SELECT * FROM volleyball WHERE date_of_match BETWEEN '$currentdate' AND '$futuredate'  ORDER BY date_of_match Desc Limit 3");
            //  SECOND	, MINUTE, HOUR, DAY, WEEK	, MONTH	, QUARTER	, YEAR,
             ...
             <nav>
                ...
                <?php for ($i= 7; $i > 0; $i--) { 
                       ...
                       $date1 = strtotime("-$i day"); 
                        if (-$i == $day) { ?>
                    <li class="page-item active"><a class="page-link px-1  text-light" href="javascript:void(0)" onclick="Volleyball_FecthRequest(<?php echo -$i; ?>,'<?php echo date('Y-m-d', $date); ?>','<?php echo date('Y-m-d', $date1); ?>')" ><?php echo date('M d', $date); ?></a></li>
                <?php }else{ ?>
                       <li class="page-item"><a class="page-link px-1  text-dark" href="javascript:void(0)" onclick="Volleyball_FecthRequest(<?php echo -$i; ?>,'<?php echo date('Y-m-d', $date); ?>','<?php echo date('Y-m-d', $date1); ?>')"><?php echo date('M d', $date);  ?></a></li>
                <?php } } ?>
                <?php for ($i= 0; $i < 7; $i++) { 
                       $date = strtotime("+$i day");
                       $date1 = strtotime("+$i day"); 
                         if ($i == $day) { ?>
                    <li class="page-item active"><a class="page-link px-1  text-light" href="javascript:void(0)" onclick="Volleyball_FecthRequest(<?php echo $i; ?>,'<?php echo date('Y-m-d', $date); ?>','<?php echo date('Y-m-d', $date1); ?>')" ><?php if ($i === 0) { echo 'Today';}else{ echo date('M d', $date); } ?></a></li>
                <?php }else{ ?>
                    <li class="page-item"><a class="page-link px-1  text-dark" href="javascript:void(0)" onclick="Volleyball_FecthRequest(<?php echo $i; ?>,'<?php echo date('Y-m-d', $date); ?>','<?php echo date('Y-m-d', $date1); ?>')" ><?php echo date('M d', $date);  ?></a></li>
                <?php } } ?>
                </ul>
