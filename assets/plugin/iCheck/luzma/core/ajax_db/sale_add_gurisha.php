    ...
    <div class="wrap6">
        ...
        <div class="img-popup-wrap">
            ...
            <div class="card">
                ...
                <form method="post" id="form-sale-gurisha"  enctype="multipart/form-data" >
                <div class="card-body">
...
if (isset($_POST['user_id']) && !empty($_POST['user_id'])) {
    ...
    if (!empty($_FILES['video']['name'][0])) {
      $video= $_FILES['video'];
      $video_ = $home->uploadSaleGurishaFile($video);
  ...
  if (!empty($_POST['photo-Title5'])) {
	...
	if (!empty($phone) || !empty(array_filter($photo['name'])) || !empty(array_filter($other_photo['name'])) ) {
		if (!empty($photo['name'][0])) {
			# code...
			$photo_ = $home->uploadSaleGurishaFile($photo);
      $other_photo_ = $home->uploadSaleGurishaFile($other_photo);
		}
	...
	$users->Postsjobscreates('sale_gurisha',array( 
	'title'=> $title,
	'code'=> $code,
	'price'=> $price,
	'phone'=> $phone,
