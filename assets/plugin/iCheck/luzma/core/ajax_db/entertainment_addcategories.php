...
if (isset($_POST['entertainment_view']) && !empty($_POST['entertainment_view'])) {
        ...
        <div class="img-popup-wrap">
            ...
            <div class="card">
                ...
                <div class="card-header">
                    <h5 class="card-text">Entertainment</h5>
                    <p class="card-text">Add News ? Please fill details below.</p>
                </div>
                <form method="post" id="form-entertainment"  enctype="multipart/form-data" >
                <div class="card-body">
                        ...
                        <div class="col">
                            <div class="form-group">
                              <select class="form-control" name="categories_news" id="categories_news">
                                ...
                                <option value="break-news">Break news</option>
...
if (isset($_POST['user_id']) && !empty($_POST['user_id'])) {
    ...
    if (!empty($_FILES['video']['name'][0])) {
      $video= $_FILES['video'];
      $video_ = $home->uploadentertainmentFile($video);
  ...
  if (!empty($_POST['photo-Title5'])) {
	...
	if (!empty($title) || !empty(array_filter($photo['name'])) || !empty(array_filter($other_photo['name'])) ) {
		if (!empty($photo['name'][0])) {
			# code...
			$photo_ = $home->uploadentertainmentFile($photo);
      $other_photo_ = $home->uploadentertainmentFile($other_photo);
		}
	...
	$users->Postsjobscreates('entertainment',array( 
	'title'=> $title,
	'authors'=> $authors,
	'photo'=> $photo_, 
